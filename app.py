...
/* Default Streamlit theme background for analysis buttons */
...
/* Hover effect */
.analysis-grid .stButton > button:hover {
    filter: brightness(1.1);
}

/* Ensure analysis button text is visible in both dark/light mode */
@media (prefers-color-scheme: light) {
  .analysis-grid .stButton > button { color: black !important; }
}
@media (prefers-color-scheme: dark) {
  .analysis-grid .stButton > button { color: white !important; }
}

/* Clear Session button styled like header but green */
div[data-testid="stSidebar"] .stButton > button {
    background: linear-gradient(90deg,#16a34a,#15803d) !important; /* green gradient */
    color: white !important;
    border-radius: 10px;
    font-weight: 600;
